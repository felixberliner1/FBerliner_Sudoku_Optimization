...
mod tests {
    use crate::all_pred;
    use crate::all_in_relation_pred;
    use crate::pred_funs;
    use crate::rel_cnf;
    ...
    fn sudoku_rules_two_somes_one_lone_one(base_struc : structure::Structure) -> rel_cnf {
        ...
        sud_rules = all_pred(vec![base_struc.size, base_struc.size, base_struc.size, base_struc.size], sud_rules, Vec::new(), &pred_funs::One(vec![base_struc.size, base_struc.size], vec![rel_literal::Pos("Cell".to_string(), vec![0,4,1,5,2,3])]), &base_struc.relations.clone());
    ...
    fn modular_line_neg_rules(base_struc : structure::Structure) -> rel_cnf {
        let mut sud_rules = all_pred(vec![base_struc.size, base_struc.size, base_struc.size, base_struc.size], Vec::new(), Vec::new(), &pred_funs::Lone(vec![base_struc.size, base_struc.size], vec![rel_literal::Pos("Cell".to_string(), vec![0,1,2,3,4,5])]), &base_struc.relations.clone());
        sud_rules = all_pred(vec![base_struc.size, base_struc.size, base_struc.size, base_struc.size], sud_rules, Vec::new(), &pred_funs::Some(vec![base_struc.size, base_struc.size], vec![rel_literal::Pos("Cell".to_string(), vec![0,1,4,5,2,3])]), &base_struc.relations.clone());
        sud_rules = all_pred(vec![base_struc.size, base_struc.size, base_struc.size, base_struc.size], sud_rules, Vec::new(), &pred_funs::Some(vec![base_struc.size, base_struc.size], vec![rel_literal::Pos("Cell".to_string(), vec![4,5,0,1,2,3])]), &base_struc.relations.clone());
        sud_rules = all_pred(vec![base_struc.size, base_struc.size, base_struc.size, base_struc.size], sud_rules, Vec::new(), &pred_funs::Some(vec![base_struc.size, base_struc.size], vec![rel_literal::Pos("Cell".to_string(), vec![0,4,1,5,2,3])]), &base_struc.relations.clone());
        ...
        sud_rules = all_in_relation_pred(sud_rules, "Mod Line".to_string(), Vec::new(), 
            &pred_funs::All(vec![base_struc.size, base_struc.size, base_struc.size], 
            Box::new(pred_funs::End(
            vec![rel_literal::Neg("Cell".to_string(), vec![0,1,2,3,12,14]), 
            rel_literal::Neg("Cell".to_string(), vec![4,5,6,7,13,14])]))), &base_struc.relations);
        
        sud_rules = all_in_relation_pred(sud_rules, "Mod Line".to_string(), Vec::new(), 
            &pred_funs::All(vec![base_struc.size, base_struc.size, base_struc.size], 
            Box::new(pred_funs::End(
            vec![rel_literal::Neg("Cell".to_string(), vec![0,1,2,3,12,14]), 
            rel_literal::Neg("Cell".to_string(), vec![8,9,10,11,13,14])]))), &base_struc.relations);
    
        sud_rules = all_in_relation_pred(sud_rules, "Mod Line".to_string(), Vec::new(), 
            &pred_funs::All(vec![base_struc.size, base_struc.size, base_struc.size], 
            Box::new(pred_funs::End(
            vec![rel_literal::Neg("Cell".to_string(), vec![4,5,6,7,12,14]), 
            rel_literal::Neg("Cell".to_string(), vec![8,9,10,11,13,14])]))), &base_struc.relations);
        
        sud_rules
    }

    fn pos_neg_clause_count() {
        let mut pos_constraint = all_pred(vec![3, 3, 3, 3], Vec::new(), Vec::new(), &pred_funs::Some(vec![3, 3], vec![rel_literal::Pos("Cell".to_string(), vec![0,4,1,5,2,3])]), &std::collections::HashMap::new());

        println!("pos constraint clauses: {}", pos_constraint.len());

        let mut neg_constraint = all_pred(vec![3, 3, 3, 3], Vec::new(), Vec::new(), &pred_funs::Lone(vec![3, 3], vec![rel_literal::Pos("Cell".to_string(), vec![0,4,1,5,2,3])]), &std::collections::HashMap::new());

        println!("neg constraint clauses: {}", neg_constraint.len());
    }

    #[test]
    fn test_sud_0some() {
        let test_1_cnf = sudoku_rules_four_ones(sudoku_x_clues(45));

        println!("{:?}", test_1_cnf.len())
    }

    #[test]
    fn test_sud_1some() {
        let test_2_cnf = sudoku_rules_two_ones_one_lone_some(sudoku_x_clues(45));

        println!("{:?}", test_2_cnf.len())
    }

    #[test]
    fn test_sud_2some() {
        let test_3_cnf = sudoku_rules_two_somes_one_lone_one(sudoku_x_clues(45));

        println!("{:?}", test_3_cnf.len())
    }

    #[test]
    fn test_sud_3some() {
        let test_4_cnf = sudoku_rules_three_somes_one_lone(sudoku_x_clues(45));

        println!("{:?}", test_4_cnf.len())
    }

    #[test]
    fn test_modline_correctness() {
        let test_modline = modular_line_neg_rules(mod_sudoku_xy_clues(0, 24));

        println!("{:?}", test_modline.len())
    }

    #[test]
    fn test_posneg_clause_count() {
        pos_neg_clause_count();
    }
}
